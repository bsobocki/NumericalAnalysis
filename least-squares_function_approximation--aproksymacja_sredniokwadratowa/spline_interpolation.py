...
class Spline_Interpolation:
    def __init__(self,x,y):
        ...
        self.Momoents = Spline_Interpolation.table_of_moments(x,y)
    ...
    def __call__(self,xx):
        for i in range(1,len(self.x)):
            ...
            if xx <= self.x[i]:
                return Spline_Interpolation.s(i, self.x, self.y, self.Momoents,xx)
        return Spline_Interpolation.s(len(self.x)-1, self.x, self.y, self.Momoents,xx)
    ...
    def p_q_table(x,y):
        ...
        n = len(x)-1
        f = Spline_Interpolation.f_table(x,y)
        p,q = [None],[None]
        p += [3 * f[2][2]]                              # p1
    ...
    def table_of_moments(x,y):
        ...
        Momoents[n-1] = 0
        Momoents[0] = 0
        p,q = Spline_Interpolation.p_q_table(x,y)
